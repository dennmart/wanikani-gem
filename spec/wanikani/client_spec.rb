...
RSpec.describe Wanikani::Client do
  ...
  describe "#initialize" do
    ...
    it "raises an ArgumentError if attempting to set an invalid API version" do
      expect {
        described_class.new(api_key: "my-api-key", api_revision: "bad-api-revision")
      }.to raise_error(ArgumentError, "API revision should be one of the following: #{Wanikani::VALID_API_REVISIONS.join(', ')}.")
    end

    it "uses the default API version if it's not set on initialization" do
      client = described_class.new(api_key: "my-api-key")
      expect(client.api_revision).to eq(Wanikani::DEFAULT_API_REVISION)
    end
  end
